...
# the CM image. It does not produce a runnable application image itself.
...
COPY nuget.config Directory.Build.targets Packages.props /nuget/
COPY src/ /temp/
...
# Restore the production library and Docker publishing project.
RUN nuget restore .\src\SitecoreJobs.Foundation\SitecoreJobs.Foundation.csproj -Verbosity quiet; `
    nuget restore .\src\Build\HelixPublishingPipeline\HPP.Platform\HPP.Platform.csproj -Verbosity quiet

# Copy remaining source code
COPY src/ ./src/

# Build the Sitecore platform artifacts.
RUN msbuild .\src\Build\HelixPublishingPipeline\HPP.Platform\HPP.Platform.csproj /p:Configuration=$env:BUILD_CONFIGURATION /m /p:DeployOnBuild=true /p:PublishProfile=Local

# Save the artifacts for copying into the CM image.
